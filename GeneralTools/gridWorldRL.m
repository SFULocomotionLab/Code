...
plot(posVec,'bo');
title('Position of learner in the grid world after each step')
xlabel('Number of steps')
ylabel('Single index value postion')
...
hold on
title('Grid world')
X=linspace(1,4,len); hax.XLim=[0 len];
Y=linspace(1,4,len); hax.YLim=[0 len];
textActualPosX = X-0.9; textMeasPosX = X-0.9;
textActualPosY = Y-0.9; textMeasPosY = Y-0.1;
% Horizontal grid 
for k = 1:length(X)
  line(hax.XLim, [Y(k) Y(k)])
end
% Vertical grid
for k = 1:length(Y)
  line([X(k) X(k)], hax.YLim)
end
axis square
xticklabels({'','1','','2','','3','','4'})
yticklabels({'','4','','3','','2','','1'})
for j=1:len
    for k=1:len
        text(hax,textActualPosX(j),textActualPosY(k),strcat('ActualCost=',num2str(costGrid(len+1-j,k))))
        hax2t(j,k)=text(hax,textMeasPosX(j),textMeasPosY(k),strcat('MeasCost=',num2str(costUpdateInit(len+1-j,k))));
    end
end
...
for i=1:maxIter
    ...
    switch actionChoiceVec(i)
        case 1
            line(hax,[newPosy-0.5 newPosy-0.5], [len+1-(newPosx+1)-0.5 len+1-(newPosx+1)-0.3],'Color','b')
            plot(hax,newPosy-0.5,len+1-(newPosx+1)-0.3,'b^')
        case 2
            line(hax,[newPosy-0.5 newPosy-0.5], [len+1-(newPosx-1)-0.5 len+1-(newPosx-1)-0.7],'Color','b')
            plot(hax,newPosy-0.5,len+1-(newPosx-1)-0.7,'bv')
        case 3
            line(hax,[newPosy+1-0.5 newPosy+1-0.9], [len+1-newPosx-0.5 len+1-newPosx-0.5],'Color','b')
            plot(hax,newPosy+1-0.9,len+1-newPosx-0.5,'b<')
        case 4
            line(hax,[newPosy-1-0.5 newPosy-1-0.1], [len+1-newPosx-0.5 len+1-newPosx-0.5],'Color','b')
            plot(hax,newPosy-1-0.1,len+1-newPosx-0.5,'b>')
        case 5
            plot(hax,newPosy-0.5, len+1-newPosx-0.5,'bs','MarkerSize',10)
    end
    if i==maxIter
        if newPos==1 || newPos==16
            plot(hax,newPosy-0.5, len+1-newPosx-0.5,'gp','MarkerSize',20,'MarkerFaceColor','g')
        else
            plot(hax,newPosy-0.5, len+1-newPosx-0.5,'rp','MarkerSize',20,'MarkerFaceColor','r')
        end
    end
